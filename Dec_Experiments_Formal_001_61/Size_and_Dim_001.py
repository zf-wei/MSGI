...
def perform_node2vec_embedding(graph, embedding_dimension,_):
    ...
    node2vec_model = Node2Vec(graph, dimensions=embedding_dimension, walk_length=16, num_walks=8, workers=32, quiet=True) #, temp_folder='test' # Use temp_folder for big graphs
    # Embed nodes 
...
def OneRound(method_id, itera = 5):
    ...
    return RECORD
def MoreRound(method_id, itera, num_workers=2):
    with concurrent.futures.ProcessPoolExecutor(max_workers=num_workers) as executor:
        futures = [executor.submit(OneRound, method_id, itera) for _ in range(num_workers)]
    ...
    for result in results:
        RECORD_parallel.extend(result)
    #RECORD_parallel = [item for sublist in results for item in sublist] 
    return RECORD_parallel